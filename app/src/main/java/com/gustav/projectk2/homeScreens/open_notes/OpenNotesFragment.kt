...
class OpenNotesFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val binding: FragmentOpenNotesBinding = DataBindingUtil.inflate(inflater, R.layout.fragment_open_notes, container, false)
        ...
        val openNotesViewModel = ViewModelProvider(requireActivity(), viewModelFactory).get(OpenNoteViewModel::class.java)

        binding.viewModel = openNotesViewModel

        val adapter = NoteAdapter(ItemClickListener { noteId -> openNotesViewModel.onSelectNoteItemClicked(noteId)
        ...
        binding.newNoteButton.setOnClickListener {
             this.findNavController().navigate(HomeViewPagerFragmentDirections.actionHomeViewPagerFragmentToTemplateFragment())
        }
